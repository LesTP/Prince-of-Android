...
// === REPLAY ORACLE INSTRUMENTATION ===
// Phase 1 spike: dump game state each frame for determinism verification
#ifdef DUMP_FRAME_STATE
static FILE* state_trace_file = NULL;
static dword frame_number = 0;

void dump_frame_state() {
	if (state_trace_file == NULL) {
		state_trace_file = fopen("state_trace.bin", "wb");
		if (state_trace_file == NULL) {
			fprintf(stderr, "ERROR: Could not open state_trace.bin for writing\n");
			return;
		}
	}

	// Write frame number
	fwrite(&frame_number, sizeof(frame_number), 1, state_trace_file);

	// Write character structs (15 fields each, 17 bytes)
	fwrite(&Kid, sizeof(char_type), 1, state_trace_file);
	fwrite(&Guard, sizeof(char_type), 1, state_trace_file);
	fwrite(&Char, sizeof(char_type), 1, state_trace_file);

	// Write key scalar state
	fwrite(&current_level, sizeof(current_level), 1, state_trace_file);
	fwrite(&drawn_room, sizeof(drawn_room), 1, state_trace_file);
	fwrite(&rem_min, sizeof(rem_min), 1, state_trace_file);
	fwrite(&rem_tick, sizeof(rem_tick), 1, state_trace_file);
	fwrite(&hitp_curr, sizeof(hitp_curr), 1, state_trace_file);
	fwrite(&hitp_max, sizeof(hitp_max), 1, state_trace_file);
	fwrite(&guardhp_curr, sizeof(guardhp_curr), 1, state_trace_file);
	fwrite(&guardhp_max, sizeof(guardhp_max), 1, state_trace_file);

	// Write current room tile state (if loaded)
	if (curr_room_tiles != NULL) {
		fwrite(curr_room_tiles, sizeof(byte), 30, state_trace_file);
	} else {
		byte zeros[30] = {0};
		fwrite(zeros, sizeof(byte), 30, state_trace_file);
	}
	if (curr_room_modif != NULL) {
		fwrite(curr_room_modif, sizeof(byte), 30, state_trace_file);
	} else {
		byte zeros[30] = {0};
		fwrite(zeros, sizeof(byte), 30, state_trace_file);
	}

	// Write trobs array (animated tiles)
	fwrite(&trobs_count, sizeof(trobs_count), 1, state_trace_file);
	fwrite(trobs, sizeof(trob_type), TROBS_MAX, state_trace_file);

	// Write mobs array (movable objects / falling tiles)
	fwrite(&mobs_count, sizeof(mobs_count), 1, state_trace_file);
	fwrite(mobs, sizeof(mob_type), 14, state_trace_file);

	// Write RNG state for verification
	fwrite(&random_seed, sizeof(random_seed), 1, state_trace_file);

	frame_number++;
	fflush(state_trace_file);
}

void close_state_trace() {
	if (state_trace_file != NULL) {
		fclose(state_trace_file);
		state_trace_file = NULL;
	}
}
#endif // DUMP_FRAME_STATE
// === END REPLAY ORACLE INSTRUMENTATION ===
