...
- **Git over NTFS is unusably slow from Windows:** `git status`, `git add`, and `git commit` on the P: drive (NTFS USB via SMB) routinely time out (30-120s+) or hang. Failed commits leave stale `.git/index.lock` files. **Always run git operations from the Pi** (`ssh pirozhok`, then `cd /mnt/passport/shared/PoP_port`). If a stale lock appears: `rm .git/index.lock`. Never run git from the Windows side on the shared drive.
...
- **Status:** COMPLETE 2026-05-02 — `LevelScreenshotGenerator` loads level resources/catalogs, maps reachable rooms with C-compatible BFS, renders rooms via `Seg008.drawRoom()` + `RenderTableFlusher`, writes `build/render/level_01_kotlin.png`, and full `gradle test --no-daemon` passes with 654 tests.
